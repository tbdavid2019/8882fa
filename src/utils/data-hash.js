...
export async function saveDataHash(env, secrets, options = {}) {
	...
	try {
		...
		// 本次備份的資料快照不晚於 backupStartedAt；更早 stage 的 pending hash 必然已被本次備份覆蓋
		// （例如分片匯入裡前幾片 stage 的中間態），清理後可避免 KV 里長期殘留陳舊 pending。
		// 未提供 backupStartedAt 時退回原有"僅雜湊一致時清理"的保守策略。
