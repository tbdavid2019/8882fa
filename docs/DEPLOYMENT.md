...
本文件提供完整的部屬與配置教學，涵蓋兩種部署方式：

- **[一鍵部署](#一鍵部署)** — 點擊按鈕即可完成，適合大多數使用者
...
本專案為公開開源儲存庫（Public Repo）。為保障個人資訊安全與代碼純淨：

- **`wrangler.toml`** 保持純淨通用配置，絕不寫死任何個人的 Cloudflare `account_id` 或私人自訂網域。
- **提供範本**：
  - `wrangler.toml.template`：完整的 Cloudflare Workers 配置範本與參數說明。
  - `.env.example`：本地部署環境變數範本。
- **本地部署自動化**：
  複製 `.env.example` 為 `.env`（已列入 `.gitignore`，絕不上傳 GitHub）。執行 `npm run deploy` 時，部署腳本會自動讀取 `.env` 並動態套用 `CLOUDFLARE_ACCOUNT_ID` 與 `CUSTOM_DOMAIN`，部署完成後立即無條件還原乾淨的 `wrangler.toml`。
...
1. 點擊上方部署按鈕，使用 GitHub 登入並授權
2. 登入 Cloudflare 帳戶，點擊 **Deploy** 等待部署完成
...
> 若您在 Cloudflare Dashboard 中設定 Git 自動建置，**部署指令請使用 `npm run deploy`，不要直接寫 `npx wrangler deploy`**，以保留版本注入與自訂網域綁定流程。
...
1. 產生加密金鑰（任選一種方式）：
   ...
   無命令列環境時，也可以在瀏覽器按 `F12` 開啟主控台（Console）執行：

   ```javascript
   btoa(String.fromCharCode(...crypto.getRandomValues(new Uint8Array(32))));
   ```

2. 在 Cloudflare Dashboard 中打開您的 Worker → **Settings** → **Variables and Secrets** → **Add**：
   - **Type**: `Secret`
   - **Variable name**: `ENCRYPTION_KEY`
   - **Value**: 貼上剛才產生的金鑰
   - 點擊 **Save and deploy**

⚠️ **重要**：`ENCRYPTION_KEY` 是解密資料的唯一主金鑰，Cloudflare 儲存後不會再次顯示原值。請立即將其保存到密碼管理器——**若遺失將無法恢復已加密的資料**。
...
2. 確認密碼後點擊「完成設定」，自動登入進入主頁
3. 登入後可在頂部安全橫幅點擊「立即啟用 Passkey」，綁定 Touch ID / Face ID 快速免密登入
...
**適合對象**：熟悉命令列，本機已安裝 [Node.js](https://nodejs.org/)（LTS 版本）和 [Git](https://git-scm.com/)。
...
# 自訂網域（可選：自動綁定並預熱自訂網域，消除 403 路由空窗）
CUSTOM_DOMAIN=2fa.yourdomain.com
```

### 步驟 4：建立 KV 命名空間（若由腳本自動處理可跳過）
...
每次執行 `npm run deploy` 時，腳本會於記憶體中動態將自訂網域宣告進部署設定：
...
這樣 Cloudflare 在部署當下就會以原子操作綁定邊緣網域路由，徹底杜絕 403 斷層，同時公開的 `wrangler.toml` 依然維持乾淨。
...
| `ONEDRIVE_CLIENT_ID`         | 選填 | OneDrive OAuth 用戶端 ID                       | Microsoft Entra 應用程式註冊                                                  |
| `ONEDRIVE_CLIENT_SECRET`     | 選填 | OneDrive OAuth 用戶端密碼                      | Microsoft Entra 應用程式註冊                                                  |
| `GOOGLE_DRIVE_CLIENT_ID`     | 選填 | Google Drive OAuth 用戶端 ID                   | Google Cloud Console                                                          |
| `GOOGLE_DRIVE_CLIENT_SECRET` | 選填 | Google Drive OAuth 用戶端密碼                  | Google Cloud Console                                                          |
...
> 正常升級**請勿**刪除 Worker 或 KV 命名空間，已設定的 Secrets（含 `ENCRYPTION_KEY`）會繼續保留。
...
刪除 KV 中的密碼雜湊即可重新設定（不影響已儲存的 2FA 密鑰）：

- **Dashboard 方式**：進入 **Workers & Pages** → **KV** → 選擇您的命名空間 → 刪除 `user_password` 鍵。
- **命令列方式**：
  ```bash
...
檢查 Worker **Settings** → **Bindings** 中是否存在名稱完全相符且區分大小寫的 `SECRETS_KV` 綁定。
